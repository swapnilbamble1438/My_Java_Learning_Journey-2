...
public class StudentDAO 
{
	...
	// search and get Students by name
	public String searchStudentByName(String name)
	{
		
        StringBuilder studentsArray = null;
		...
		ResultSet rs = ps.executeQuery();
		studentsArray =  new StringBuilder("[");
		while(rs.next())
		{

			String student = "{'id':" + rs.getInt(1)    + ",'"
							  +"name':'" + rs.getString(2) + "','"
							  +"dob':'" + rs.getString(3) + "','"
		...
		studentsArray = new StringBuilder(studentsArray.substring(0,studentsArray.length()-1));

		studentsArray.append("]");	
		
		studentsArray = new StringBuilder(studentsArray.toString().replace("'",Character.toString('"')));
